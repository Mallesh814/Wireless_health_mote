...
uint32_t ADS1294_Init(uint32_t ui32Base, uint32_t ui32BitRate){
	...
	while(i--);
	buffer[0] = RESET;
	status = SPI_Write(SSI3_BASE, buffer, 0);
	...
	status = SPI_Write(SSI3_BASE, buffer, 2);
	...
	*/
	...
	status = SPI_Write(SSI3_BASE, buffer, 5);
